...
namespace RazorLogin.Pages.Manag.Report2
{
    public class IndexModel : PageModel
        {
            ...
            public async Task OnGetAsync(int page = 1, string search = null)
            {
                ...
                var query = _context.GiftShops.AsQueryable();

                if (!string.IsNullOrEmpty(search))
                {
                    query = query.Where(fs => fs.GiftShopName.Contains(search) || fs.ShopId.ToString() == search);
                }
                ...
                StoreReports = await query
                    ...
                    {
                        FoodStoreId = fs.ShopId,
                        FoodStoreName = fs.GiftShopName,
                ...
                if (string.IsNullOrEmpty(search))
                {
                    ...
                    StoreWithMinAvgSalary = await _context.GiftShops
                        .Select(fs => new StoreReport
                        {
                            FoodStoreId = fs.ShopId,
                            FoodStoreName = fs.GiftShopName,
                    ...
                    StoreWithMaxAvgSalary = await _context.GiftShops
                        .Select(fs => new StoreReport
                        {
                            FoodStoreId = fs.ShopId,
                            FoodStoreName = fs.GiftShopName,
